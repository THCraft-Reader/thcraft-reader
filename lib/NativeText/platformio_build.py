...
if env.subst("$PIOENV") not in pro_environments:
    raise RuntimeError("NativeText must be excluded from non-X4-Pro environments")
...
env.BuildSources(str(Path(env.subst("$BUILD_DIR")) / "native-assets"), str(asset_directory), "+<*.generated.cpp>")
